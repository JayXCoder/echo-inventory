...
function adduser()
{
	...
	$empnom = trim(htmlspecialchars($_POST['empnom']));
	...
	if (mysqli_num_rows($query1)==0) {
		$sql = "INSERT INTO `users` VALUES ('$username','$pass','$fname','$sname','$cemail','$empnom','$type')";
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
...
function updateuser()
{
	...
	$name = $_GET['name'];
		...
		$sql = "UPDATE `users` SET `fname`='$fname',`sname`='$sname',`cemail`='$cemail',`empnom`='$empnom',`type`='$type',`password`='$pass' WHERE `username`='$name'";
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
